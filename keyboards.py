...
def phone_button():
    return ReplyKeyboardMarkup([
        [KeyboardButton(text='Raqam jo`natish ☎️', request_contact=True)]
...
def generate_category_menu():
    ...
    markup.row(
        InlineKeyboardButton(text='MENU 🧾', url='https://telegra.ph/Oshxona-10-05')
    )
...
def generate_cart_menu(cart_id):
    ...
    for cart_product_id, products_name in cart_products:
        markup.row(
            InlineKeyboardButton(text=f'❌ {products_name}', callback_data=f'delete_{cart_product_id}')
        )
    return markup